...
from penny_v2.core.events import (
    AppShutdownEvent, UILogEvent,
    SearchRequestEvent, SearchResultEvent
)
...
class SearchService:
    ...
    async def handle_search_request(self, event: SearchRequestEvent):
        ...
        await self.event_bus.publish(SearchResultEvent(
            ...
            results=results,
            source=event.source,
            original_user=event.original_user, # Pass through
            original_context=event.original_context, # Pass through
